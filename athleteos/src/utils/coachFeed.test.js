...
describe("file d'actions du coach", () => {
  it("place les tâches concrètes avant les analyses descriptives et conserve leur destination", () => {
      ...
      sessions: [
        {
          ...
          athleteIds: [7], validations: [{ athleteId: 7, rsvpStatus: null }],
        },
        {
          id: 5, week: 31, sessionDate: "2026-07-28", createdByAthlete: false,
          athleteIds: [7], validations: [{ athleteId: 7, rsvpStatus: "going" }],
        },
      ],
    });
    ...
    expect(items.slice(0, 5).map(item => item.route)).toEqual([
      "alerts", "planning", "athletes", "planning", "competitions",
    ]);
  });
