...
export default function ModuleOnboardingModal() {
  ...
  const [selectedClubKeys, setClubKeys] = useState(null);
  const clubKeys = useMemo(() => selectedClubKeys ?? MODULE_KEYS.filter(key => club[key] !== false), [club, selectedClubKeys]);
  ...
  const visible = (!loading && !configuredAt) || saving || Boolean(error);
  const { dialogRef } = useAccessibleDialog({ enabled: visible, closeDisabled: true });
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center sm:p-4 modal-backdrop">
      <section ref={dialogRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="module-onboarding-title"
        ...
        <header className="px-6 py-5 flex items-start gap-3" style={{ borderBottom: "1px solid var(--c-border)" }}>
          ...
          <div>
            <p className="meta-text">{hasAthletes ? `ÉTAPE ${step} SUR 2` : "BIENVENUE DANS TON CLUB"}</p>
            ...
            <p className="text-[13px] mt-1" style={{ color: "var(--c-text-2)" }}>
              {step === 1 ? "Choisis une base pour commencer. Tu pourras tout modifier plus tard dans Réglages → Outils." : "Les outils du club sont déjà sélectionnés. Tu pourras les adapter depuis Athlètes."}
            </p>
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <details>
          <summary className="min-h-11 flex items-center cursor-pointer text-[13px] font-semibold">Personnaliser les outils · Plus d’options</summary>
          <ModuleSelector
        ...
        <footer className="px-6 py-4 flex justify-between gap-3" style={{ borderTop: "1px solid var(--c-border)" }}>
          ...
          {step === 1 && hasAthletes
            ? <button type="button" className="btn-primary" disabled={saving} onClick={() => { setAthleteKeys(clubKeys); setStep(2); }}>Continuer <ArrowRight size={15} /></button>
