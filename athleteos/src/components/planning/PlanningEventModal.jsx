...
export default function PlanningEventModal({ athletes, initialData = null, initialKind = "stage", onClose, onSave, onDelete }) {
  ...
  return <div className="modal-backdrop fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={event => event.target === event.currentTarget && !saving && onClose()}>
    <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1} className="modal-content w-full sm:max-w-lg max-h-[94dvh] rounded-t-3xl sm:rounded-3xl overflow-hidden flex flex-col">
      ...
      <form className="flex-1 min-h-0 flex flex-col" onSubmit={submit}>
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">{KINDS.map(kind => <button type="button" key={kind.id} onClick={() => set("kind", kind.id)} className={form.kind === kind.id ? "btn-primary" : "btn-secondary"}>{kind.label}</button>)}</div>
        ...
        <footer className="p-4 pb-[max(16px,var(--safe-bottom))] flex items-center justify-between gap-3" style={{ borderTop:"1px solid var(--c-border)" }}>
