...
// même animation que l'ancien anneau "Comment tu te sens", mais le centre
// affiche un mot de statut (Faible/Modéré/Bon/Élevé/Très élevé) au lieu d'un
// chiffre brut : le chiffre reste disponible dans le détail au tap.
// Rendu en carte compacte (comme les autres tuiles de l'app) plutôt qu'en
// anneau flottant nu, pour rester net dans une bande de 5 jauges.
const TrainingGauge = memo(({ value, color = "var(--c-text-3)", statusWord, label, size = 64, onClick }) => {
  ...
  return (
    <button type="button" onClick={onClick} className="tap-feedback"
      ...
      <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ filter: hasValue ? `drop-shadow(0 0 8px ${color}30)` : "none" }}>
